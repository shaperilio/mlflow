...
export const ExperimentViewRunsTable = React.memo(
  ...
  ({
    ...
    const previousClientSortRef = useRef<typeof clientSort>(null);

    // Drive AG Grid's own sort (using the column's numeric comparator) from the local clientSort
    // state, and refresh the headers so the sort arrow updates. When a client sort is cleared, drop
    // AG Grid's sort so the server-provided order stands. With no client sort before or now this is
    // a no-op, so it doesn't touch the grid's column state (e.g. the persisted layout) on every
    // columnDefs change.
    useEffect(() => {
      if (!columnApi) {
        return;
      }
      const hadClientSort = previousClientSortRef.current !== null;
      previousClientSortRef.current = clientSort;
