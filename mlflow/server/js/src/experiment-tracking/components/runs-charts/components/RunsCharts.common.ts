  ...
  return runsData.flatMap((runEntry): LegendLabelData[] => {
    ...
    if (shouldEnableChartExpressions() && yAxisKey === RunsChartsLineChartYAxisType.EXPRESSION) {
      ...
      leftEntries = metricKeys.map((metric, idx) => {
        const customLineStyle = getCustomLineStyle?.(metric);
        return {
 ...
 * from the data extent of the given metrics. The plot and its zoom/reset handling work with a
 * concrete range pair, so we compute the fill here at the data layer (memoized on the sampled data)
 * rather than on every chart render. Returns undefined when neither bound is set. `isLog` => the
 * user bounds are in log10 space, so the filled data extent is logged to match.
 */
